...
void AWarriorSurvivalGameMode::PreLoadNextWaveEnemies()
{
	...
	for (const FWarriorEnemyWaveSpawnerInfo& SpawnerInfo : GetCurrentWaveSpawnerTableRow()->EnemyWaveSpawnDefinitions)
	{
...
int32 AWarriorSurvivalGameMode::TrySpawnWaveEnemies()
{
	...
	for (const FWarriorEnemyWaveSpawnerInfo& SpawnerInfo : GetCurrentWaveSpawnerTableRow()->EnemyWaveSpawnDefinitions)
	{
		...
		for (int32 i = 0; i < NumToSpawn; i++)
		{
			...
			UNavigationSystemV1::K2_GetRandomLocationInNavigableRadius(this, SpawnOrigin, RandomLocation, 600.f);
...
void AWarriorSurvivalGameMode::OnEnemyDestroyed(AActor* DestroyedActor)
{
	...
	//Debug::Print(TEXT("CurrentSpawnEnemiesCounter: %i, TotalSpawnedEnemiesThisWaveCounter: %i"), CurrentSpawnEnemiesCounter, TotalSpawnedEnemiesThisWaveCounter);
